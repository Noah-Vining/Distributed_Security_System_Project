...
public class FishGenerator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // direct transforms or transformArray is read only
        ...
        for (int i = 0; i < amountOfFish; i++)
        {
            ...
            //Spawn off the ground at a height and in a random X and Z position with affecting affecting height
            Vector3 spawnPoint = (transform.position + Vector3.up * spawnHeight) + new Vector3(distanceX, distanceY, distanceZ);
    ...
    struct PositionUpdateJob : IJobParallelForTransform
    {
        ...
        public void Execute(int i, TransformAccess transform)
        {
            ...
            {
                Vector3 internalPosition = new Vector3(center.x + randomGen.NextFloat(-bounds.x / 2, bounds.x / 2)/1.3f, center.y + randomGen.NextFloat(-bounds.y / 2, bounds.y / 2) / 1.3f, center.z + randomGen.NextFloat(-bounds.z / 2, bounds.z / 2)/1.3f);
            ...
            if (randomise)
            {
                ...
                {
                    objectVelocities[i] = new Vector3(randomGen.NextFloat(-1f, 1f), randomGen.NextFloat(-1f, 1f), randomGen.NextFloat(-1f, 1f));
                }
            }
        }
    }
